...
public class DaoUserImpl implements DaoUser {
    ...
    public List<User> findUsersByGroup(String groupName) {
        try (Session session = HibernateFactory.getSessionFactory().getCurrentSession()) {
            Transaction transaction = session.beginTransaction();
            List<User> usersByGroup = session.createQuery("SELECT u from User u WHERE u.group.group = :groupName",
    ...
    public Optional<User> findUserById(int id) {
        try (Session session = HibernateFactory.getSessionFactory().getCurrentSession()) {
            Transaction transaction = session.beginTransaction();
            List<User> usersById = session.createQuery("SELECT u from User u WHERE u.id = :id",
    ...
    public Optional<User> findUserByLogin(String login) {
        try (Session session = HibernateFactory.getSessionFactory().getCurrentSession()) {
            Transaction transaction = session.beginTransaction();
            List<User> usersByLogin = session.createQuery("SELECT u from User u WHERE u.login = :login",
